...
App.views.eliminarInsumo = async function (insumoId) {
    const item = (App.state.inventario || []).find(x => x.id === insumoId);
    if (!item) return;
    const ok = window.confirm(`¿Eliminar el insumo ${item.nombre || insumoId}?`);
    if (!ok) return;
                ...
                <div class="dm-list-card tarj-inv">
                    <div style="display:flex; flex-direction:column; gap:10px;">
                        ...
                        <div class="dm-list-card-actions" style="display:flex; gap:8px; flex-wrap:wrap;">
                            ...
                            <button class="dm-btn dm-btn-danger dm-btn-sm" onclick="App.views.eliminarInsumo('${i.id}')">🗑️ Eliminar</button>
                        </div>
                    </div>
                </div>
            `;
...
App.views.compras = function() {
    ...
    let html = `
        <div class="dm-section" style="padding-bottom:90px;">
            <div class="dm-mb-4" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(170px,1fr)); gap:12px;">
                ...
                <div class="dm-card"><small class="dm-muted">Pendiente</small><div class="dm-text-xl dm-fw-bold" style="color:${totalPendiente > 0 ? 'var(--dm-danger)' : 'var(--dm-success)'};">${App.ui.money(totalPendiente)}</div></div>
            </div>

            <div class="dm-card dm-mb-4">
                <div style="display:flex; flex-direction:column; gap:10px;">
                    <div>
                        <h3 class="dm-card-title">Compras registradas</h3>
                        <p class="dm-muted" style="margin-top:6px;">Control de compras a proveedor y abonos.</p>
                    </div>
                    <input type="text" id="bus-compras" class="dm-input" onkeyup="window.filtrarLista('bus-compras', 'tarj-compra')" placeholder="🔍 Buscar compra o proveedor...">
                </div>
            </div>
    ...
    if (!compras.length) {
        ...
        compras.forEach(c => {
            ...
            const pagado = parseFloat(c.monto_pagado || 0) || 0;
            const saldo = Math.max(0, total - pagado);
            const fecha = c.fecha ? String(c.fecha).split('T')[0] : '';
            const badgeClass = saldo > 0 ? 'dm-badge-danger' : 'dm-badge-success';

            html += `
                <div class="dm-list-card tarj-compra">
                    <div style="display:flex; flex-direction:column; gap:10px;">
                        <div class="dm-row-between" style="align-items:flex-start; gap:12px; flex-wrap:wrap;">
                            <div style="flex:1; min-width:0;">
                                <div class="dm-list-card-title">${App.ui.safe(c.id)}</div>
                                <div class="dm-list-card-subtitle">${App.ui.safe(prov.nombre || 'Proveedor')}</div>
                        ...
                        <div class="dm-card" style="background:var(--dm-surface-2); padding:10px;">
                            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(100px,1fr)); gap:10px; text-align:center;">
                                ...
                                <div><small class="dm-muted">Pagado</small><br><strong style="color:var(--dm-success);">${App.ui.money(pagado)}</strong></div>
                        ...
                        <div class="dm-list-card-actions" style="display:flex; gap:8px; flex-wrap:wrap;">
                            ...
                            <button class="dm-btn dm-btn-danger dm-btn-sm" onclick="App.logic.eliminarCompra('${c.id}')">🗑️ Eliminar</button>
                        </div>
                    </div>
                </div>
            `;
...
App.views._optionsProveedorCompra = function () {
    ...
    (App.state.proveedores || []).forEach(p => { htmlProv += `<option value="${p.id}">${App.ui.safe(p.nombre)}</option>`; });
    return htmlProv;
};

App.views._optionsMaterialCompra = function () {
    let htmlMat = '<option value="">-- Material / reventa --</option>';
    (App.state.inventario || []).forEach(m => { htmlMat += `<option value="${m.id}">${App.ui.safe(m.nombre)} (${App.ui.safe(m.tipo || '')})</option>`; });
    return htmlMat;
};
    ...
    return `
        <div class="fila-compra-item dm-card" style="background:var(--dm-surface-2); padding:10px; margin-bottom:10px;">
            <div style="display:grid; grid-template-columns:1fr; gap:10px;">
                <select class="dm-select" name="mat_id[]" onchange="App.views.calcularTotalCompraForm()">${App.views._optionsMaterialCompra()}</select>
...
App.views.formProveedorCompra = function (callback = null) {
    const formHTML = `
        <form id="dynamic-form">
            <div class="dm-form-group"><label class="dm-label">Nombre del proveedor</label><input type="text" class="dm-input" name="nombre" required></div>
            <div class="dm-form-group"><label class="dm-label">Teléfono</label><input type="text" class="dm-input" name="telefono"></div>
            <div class="dm-form-group"><label class="dm-label">Correo</label><input type="email" class="dm-input" name="correo"></div>
...
App.views.formMaterialCompra = function (callback = null) {
    const formHTML = `
        <form id="dynamic-form">
            <div class="dm-form-group"><label class="dm-label">Nombre</label><input type="text" class="dm-input" name="nombre" required></div>
            <div class="dm-form-row" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(160px,1fr)); gap:12px;">
                <div class="dm-form-group"><label class="dm-label">Tipo</label><select class="dm-select" name="tipo"><option value="hilo">Hilo</option><option value="accesorio">Accesorio</option><option value="reventa">Reventa</option></select></div>
                <div class="dm-form-group"><label class="dm-label">Unidad</label><select class="dm-select" name="unidad"><option value="Tubos">Tubos</option><option value="Kg">Kg</option><option value="Pzas">Pzas</option></select></div>
            </div>
            <div class="dm-form-row" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(160px,1fr)); gap:12px;">
                <div class="dm-form-group"><label class="dm-label">Stock inicial</label><input type="number" step="0.1" class="dm-input" name="stock_real" value="0"></div>
                <div class="dm-form-group"><label class="dm-label">Stock mínimo</label><input type="number" step="0.1" class="dm-input" name="stock_minimo" value="0"></div>
            </div>
...
App.views.formCompra = function() {
    const formHTML = `
        <form id="dynamic-form">
            ...
            <div class="dm-form-group"><label class="dm-label">Fecha</label><input type="date" class="dm-input" name="fecha" value="${new Date().toISOString().split('T')[0]}" required></div>
            ...
            <div class="dm-form-row" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(160px,1fr)); gap:12px;">
                <div class="dm-form-group"><label class="dm-label">Total</label><input type="number" step="0.01" class="dm-input" name="total" readonly required></div>
...
App.views.formAbonoCompra = function(compraId) {
    ...
    const formHTML = `
        <form id="dynamic-form">
            ...
            <div class="dm-alert dm-alert-info dm-mb-3">Saldo pendiente: ${App.ui.money(saldo)}</div>
            <div class="dm-form-group"><label class="dm-label">Monto a abonar</label><input type="number" step="0.01" class="dm-input" name="monto" max="${saldo}" required></div>
...
App.views.verDetallesCompra = function(compraId) {
    ...
    if (!detalles.length) html += `<div class="dm-alert dm-alert-warning">Sin detalles de compra.</div>`;
    else detalles.forEach(d => {
        html += `
            <div class="dm-list-card" style="padding:10px;">
                <div class="dm-fw-bold">${App.ui.safe(d.nombre || d.mat_id || '')}</div>
                <div class="dm-text-sm dm-muted dm-mt-2">Cantidad: <strong>${App.ui.safe(d.cantidad)}</strong></div>
                <div class="dm-text-sm dm-muted">Costo unitario: <strong>${App.ui.money(d.costo_unitario || 0)}</strong></div>
            </div>
        `;
    });
    html += `</div>`;
    App.ui.openSheet(`Compra ${compraId}`, html);
};
...
App.views.formMaterial = function(id = null, callback = null) {
    ...
    const formHTML = `
        <form id="dynamic-form">
            <div class="dm-form-group"><label class="dm-label">Nombre</label><input type="text" class="dm-input" name="nombre" value="${obj ? App.ui.escapeHTML(obj.nombre) : ''}" required></div>
            <div class="dm-form-row" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(160px,1fr)); gap:12px;">
                <div class="dm-form-group"><label class="dm-label">Tipo</label><select class="dm-select" name="tipo"><option value="hilo" ${obj && obj.tipo === 'hilo' ? 'selected' : ''}>Hilo</option><option value="accesorio" ${obj && obj.tipo === 'accesorio' ? 'selected' : ''}>Accesorio</option><option value="reventa" ${obj && obj.tipo === 'reventa' ? 'selected' : ''}>Reventa</option></select></div>
                <div class="dm-form-group"><label class="dm-label">Unidad</label><select class="dm-select" name="unidad"><option value="Tubos" ${obj && obj.unidad === 'Tubos' ? 'selected' : ''}>Tubos</option><option value="Kg" ${obj && obj.unidad === 'Kg' ? 'selected' : ''}>Kg</option><option value="Pzas" ${obj && obj.unidad === 'Pzas' ? 'selected' : ''}>Pzas</option></select></div>
            </div>
            <div class="dm-form-row" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(160px,1fr)); gap:12px;">
                <div class="dm-form-group"><label class="dm-label">Stock físico</label><input type="number" step="0.1" class="dm-input" name="stock_real" value="${obj ? obj.stock_real : '0'}" required></div>
                <div class="dm-form-group"><label class="dm-label">Stock mínimo</label><input type="number" step="0.1" class="dm-input" name="stock_minimo" value="${obj ? obj.stock_minimo : '0'}" required></div>
            </div>
