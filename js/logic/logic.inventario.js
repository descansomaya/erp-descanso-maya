...
Object.assign(App.logic, {
    ...
    async guardarAbonoCompra(datos) {
        try {
            ...
            if (res.status === "success") {
                ...
                if (sheetBg && !sheetBg.classList.contains("hidden")) {
    ...
    async eliminarCompra(id) {
        try {
            if (!confirm("⚠️ ¿Eliminar compra? Se restará el stock físico ingresado, se borrarán los pagos y el gasto asociado.")) return;

            App.ui.showLoader("Eliminando en cascada...");
            ...
            movimientosAsociados.forEach(m => {
                const material = (App.state.inventario || []).find(mat => mat.id === (m.ref_id || m.material_id));
                if (material) {
                    const nuevoReal = Math.max(0, parseFloat(material.stock_real || 0) - Math.abs(parseFloat(m.cantidad || 0)));
            ...
            if (res.status === "success") {
                ...
                App.ui.toast(res.message || "Error al eliminar compra", "danger");
            }
        } catch (error) {
