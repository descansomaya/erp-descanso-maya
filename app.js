...
const App = {
        ...
        openSheet(title, contentHTML, onSaveCallback) {
            this.container.innerHTML = `
                ...
                <div class="bottom-sheet active" id="sheet-content">
                    <div class="sheet-header"><h3>${title}</h3><button class="sheet-close" id="sheet-close">&times;</button></div>
            ...
            if(form && onSaveCallback) {
                form.onsubmit = (e) => {
                    ...
                    onSaveCallback(Object.fromEntries(formData.entries()));
                    this.closeSheet();
                };
            }
        },
    ...
    logic: {
        async cargarDatosIniciales() {
            ...
            try {
                ...
                const [resMat, resCli, resProd, resPed, resDet, resOrd, resArt] = await Promise.all([
                    ...
                    App.api.fetch("leer_hoja", { nombreHoja: "ordenes_produccion" }),
                    App.api.fetch("leer_hoja", { nombreHoja: "artesanos" })
                ]);
                ...
                App.state.ordenes_produccion = resOrd.data || [];
                App.state.artesanos = resArt.data || [];

                App.ui.hideLoader();
                App.router.init();
            } catch (error) {
                App.ui.toast("Error al cargar: " + error.message);
        ...
        async guardarNuevoPedido(datosFormulario) {
            ...
            const datosPedido = {
                id: pedidoId, cliente_id: datosFormulario.cliente_id, estado: "nuevo",
                total: totalNum, anticipo: anticipoNum, notas: datosFormulario.notas,
                fecha_entrega: datosFormulario.fecha_entrega, fecha_creacion: new Date().toISOString()
            };

            const datosDetalle = {
                id: "PDET-" + Date.now(), pedido_id: pedidoId, producto_id: datosFormulario.producto_id,
                cantidad: cantidadNum, precio_unitario: totalNum / cantidadNum 
            };
                    ...
                    html += `
                        <div class="card" style="border: 1px solid var(--border); box-shadow: none;">
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 10px;">
                                ...
                                ${tieneOrden ? `<span class="badge" style="background: #E2E8F0; color: #4A5568;">En Taller</span>` : ''}
                            </div>
                        </div>
                    `;
                });
                ...
                return `
                    <div class="kanban-card" onclick="App.ui.toast('Detalle de orden próximamente')">
                        ...
                        <div style="margin-top: 8px; font-size: 0.8rem; color: var(--text-muted);">
                            ${artesano ? '👤 ' + artesano.nombre : '👤 Sin asignar'}
                        </div>
                    </div>
                `;
            };
        ...
        formNuevoPedido() {
            ...
            const formHTML = `
                <form id="dynamic-form">
                    <div class="form-group"><label>Cliente</label><select name="cliente_id" required><option value="">-- Elige un cliente --</option>${opcionesClientes}</select></div>
                    <div class="form-group"><label>Producto</label><select name="producto_id" required><option value="">-- Elige un producto --</option>${opcionesProductos}</select></div>
                    ...
                    <div class="form-group"><label>Anticipo Pagado ($)</label><input type="number" name="anticipo" value="0" required></div>
