 ...
 * Órdenes para Stock Interno, Casilla de Bodega y Edición de Compras
 */
...
window.cargarTarifas = function(artesanoId) {
    ...
    if(!select) return;
    select.innerHTML = '<option value="">-- Seleccione Trabajo --</option>' + tarifas.map(t => `<option value="${t.monto}">${t.clasificacion} ($${t.monto})</option>`).join('');
};
...
const App = {
    ...
    logic: {
        ...
        async eliminarCompra(compraId) {
            if(!confirm("⚠️ ¿Eliminar compra? Ajusta manualmente tu inventario restando lo comprado.")) return;
        ...
        // --- ORDEN DE PRODUCCIÓN INTERNA (STOCK) ---
        async guardarOrdenStock(datosFormulario) {
            App.ui.showLoader("Metiendo a producción y apartando hilo...");
            const pedidoId = "PED-STOCK-" + Date.now();
            const cantidadNum = parseInt(datosFormulario.cantidad) || 1;
            
            // Creamos un pedido "Falso" a nombre de STOCK INTERNO
            const datosPedido = { id: pedidoId, cliente_id: "STOCK_INTERNO", estado: "nuevo", total: 0, anticipo: 0, notas: "Producción para Bodega/Stock", fecha_entrega: "", fecha_creacion: new Date().toISOString() }; 
            ...
            // Descontamos inventario como en una orden normal
            const producto = App.state.productos.find(p => p.id === datosFormulario.producto_id);
            const promesaMateriales = [];
            
            if(producto) {
                for(let i=1; i<=5; i++) {
                    const matId = producto[`mat_${i}`]; const cantTeorica = parseFloat(producto[`cant_${i}`] || 0);
            ...
            // Guardamos el Pedido, Detalle y la Orden de Producción al mismo tiempo
            ...
            if (resPedido.status === "success") { 
                ...
                App.state.pedidos.push(datosPedido); App.state.pedido_detalle.push(datosDetalle); App.state.ordenes_produccion.push(nuevaOrden);
                App.ui.hideLoader(); App.ui.toast("Orden de Stock creada exitosamente"); App.router.handleRoute(); 
        ...
        // --- CIERRE Y SUMA AL INVENTARIO DE TERMINADOS ---
        async cerrarOrdenProduccion(datosFormulario) {
            App.ui.showLoader("Cerrando orden..."); const ordenId = datosFormulario.orden_id; 
            ...
            // 2. Sumar al inventario Físico si marcaron la casilla
	...
	views: {
        ...
        mas() { return `<div class="grid-2"><div class="card stat-card" style="cursor:pointer;" onclick="App.router.navigate('clientes')"><div class="label" style="margin-top:0;">👥 Clientes</div></div><div class="card stat-card" style="cursor:pointer;" onclick="App.router.navigate('proveedores')"><div class="label" style="margin-top:0;">🚚 Proveedores</div></div><div class="card stat-card" style="cursor:pointer;" onclick="App.router.navigate('artesanos')"><div class="label" style="margin-top:0;">🧑‍🎨 Artesanos / Tareas</div></div><div class="card stat-card" style="cursor:pointer; background: #FFFBEB;" onclick="App.router.navigate('inventario')"><div class="label" style="margin-top:0; color:#D69E2E;">📦 Bodega / Insumos</div></div><div class="card stat-card" style="cursor:pointer;" onclick="App.router.navigate('productos')"><div class="label" style="margin-top:0;">🧶 Productos / Recetas</div></div><div class="card stat-card" style="cursor:pointer; background: #EBF8FF;" onclick="App.router.navigate('compras')"><div class="label" style="margin-top:0; color:#3182CE;">🛒 Historial de Compras</div></div><div class="card stat-card" style="cursor:pointer; background: #FAF5FF;" onclick="App.router.navigate('reparaciones')"><div class="label" style="margin-top:0; color:#6B46C1;">🪡 Reparaciones</div></div><div class="card stat-card" style="cursor:pointer; background: #EDF2F7;" onclick="App.router.navigate('configuracion')"><div class="label" style="margin-top:0; color: #4A5568;">⚙️ Configuración</div></div></div>`; },
        ...
        formCompra(id = null) { 
            const obj = id ? App.state.compras.find(c => c.id === id) : null;
            const opcProv = App.state.proveedores.map(p => `<option value="${p.id}" ${obj && obj.proveedor_id === p.id ? 'selected':''}>${p.nombre}</option>`).join(''); 
            let bodyHTML = '';
            if(obj) {
                bodyHTML = `<div style="background: #FEFCBF; padding: 10px; border-radius: 8px; margin-bottom: 15px; font-size: 0.85rem;"><strong>Nota:</strong> Para no desajustar el inventario, al editar solo puedes cambiar Proveedor, Fecha y Total. Si te equivocaste de cantidad o material, ELIMINA la compra y vuelve a hacerla.</div><div class="form-group"><label>Proveedor</label><select name="proveedor_id" required><option value="">-- Elige Proveedor --</option>${opcProv}</select></div><div class="form-group"><label>Fecha</label><input type="date" name="fecha" value="${obj.fecha}" required></div><div class="form-group"><label>Costo Total Pagado ($)</label><input type="number" name="total" value="${obj.total}" required></div><button type="submit" class="btn btn-primary" style="width: 100%; margin-top: 10px;">Guardar Cambios</button>`;
            } else {
                const opcMat = App.state.inventario.map(m => `<option value="${m.id}">${m.nombre} (Stock actual: ${m.stock_actual})</option>`).join(''); 
                bodyHTML = `<div class="form-group"><label>Proveedor</label><select name="proveedor_id" required><option value="">-- Elige Proveedor --</option>${opcProv}</select></div><div class="form-group"><label style="display:flex; justify-content:space-between; align-items:center;">Insumo Comprado <span style="color:var(--primary); cursor:pointer; font-weight:bold; font-size:0.8rem;" onclick="App.ui.closeSheet(); setTimeout(()=>App.views.formMaterial(), 400);">+ Crear Insumo</span></label><select name="material_id" required><option value="">-- Elige Material --</option>${opcMat}</select></div><div class="form-group"><label>Cantidad a Sumar</label><input type="number" name="cantidad" step="0.1" required></div><div class="form-group"><label>Fecha</label><input type="date" name="fecha" value="${new Date().toISOString().split('T')[0]}" required></div><div class="form-group"><label>Costo Total Pagado ($)</label><input type="number" name="total" required></div><button type="submit" class="btn btn-primary" style="width: 100%; margin-top: 10px; background: var(--success); border-color: var(--success);">Confirmar Compra y Sumar Stock</button>`;
            }
            const formHTML = `<form id="dynamic-form">${bodyHTML}</form>`; 
            App.ui.openSheet(obj ? "Editar Compra" : "Ingresar Compra", formHTML, (data) => {
                if(obj) App.logic.actualizarRegistroGenerico("compras", id, data, "compras");
                else App.logic.guardarNuevaCompra(data);
            }); 
        },
        ...
        modalEditarOrden(ordenId) { 
            ...
            pagosHTML += `<div style="margin-top: 10px; text-align: right; font-weight: bold; color: var(--primary);">Costo Mano de Obra: $${costoManoObra.toFixed(2)}</div></div><button type="button" class="btn btn-secondary" style="width:100%; margin-bottom:15px; border-style:dashed;" onclick="App.ui.closeSheet(); setTimeout(()=>App.views.formAgregarTrabajo('${ordenId}'), 300);">+ Asignar Tarea / Artesano</button>`;
