/**
 * DESCANSO MAYA ERP - Motor Principal v66
 * ACTUALIZADO: PDFs simétricos, Finanzas con Reparaciones, Historial detallado y Corrección de Tarifas.
 */
        ...
        setTimeout(() => { 
            if(window.Chart) { 
                ...
                if(ctx2) {
                    ...
                    window.graficaDona = new Chart(ctx2, { type: 'doughnut', data: { labels: Object.keys(expPorCat).map(k=>k.toUpperCase()), datasets: [{ data: Object.values(expPorCat), backgroundColor: ['#E53E3E', '#DD6B20', '#D69E2E', '#3182CE', '#805AD5'] }] }, options: { responsive: true, plugins: { legend: { position: 'bottom' } } } });
                }
            } 
        }, 300);
    },
...
App.views.pedidos = function() { 
    ...
    const dibujarTarjetaPed = (p) => {
        ...
        return `<div class="card tarj-ped" data-estado="${p._estadoCalculado}" style="border: ${esUrgente ? '2px solid var(--danger)' : '1px solid var(--border)'}; box-shadow: none;">
            <div style="display: flex; justify-content: space-between; margin-bottom: 8px;"><strong>${p.id} - ${nombreC}</strong>${estaPagado ? `<span class="badge" style="background: var(--success); color: white;">PAGADO</span>` : `<span class="badge ${p._estadoCalculado || 'nuevo'}">${String(p._estadoCalculado || 'Nuevo').toUpperCase()}</span>`}</div>
...
App.views.formEditarPedido = function(id) { 
    ...
    App.ui.openSheet("Editar Pedido", formHTML, (data) => { if(data.fecha_creacion) data.fecha_creacion = data.fecha_creacion + "T12:00:00.000Z"; App.logic.actualizarRegistroGenerico("pedidos", id, data, "pedidos"); }); 
};
...
App.views.formCerrarOrden = function(ordenId) { 
    if (typeof ordenId !== 'string') ordenId = null;
    ...
    let materialesHTML = ''; let recetaGuardada = null;
