/**
 * DESCANSO MAYA ERP - Motor Principal v28
 * Recetas Multi-Material, Tarifas por Artesano, Clasificaciones y Reventa Automática
 */
...
const App = {
    ...
    ui: {
        ...
        openSheet(title, contentHTML, onSaveCallback) {
            ...
            if(form && onSaveCallback) { form.onsubmit = (e) => { e.preventDefault(); const data = Object.fromEntries(new FormData(form).entries()); this.closeSheet(); onSaveCallback(data); }; }
        },
	...
	logic: {
        async verificarPIN(pinIngresado) {
            ...
            if (res.status === "success") { localStorage.setItem('erp_pin', pinIngresado); App.ui.toast("¡Acceso concedido!"); this.cargarDatosIniciales(); } else { App.state.pinAcceso = null; App.ui.hideLoader(); App.ui.toast("PIN Incorrecto."); }
        },
        ...
        descargarRespaldo() {
            const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(App.state, null, 2)); const dlAnchorElem = document.createElement('a'); dlAnchorElem.setAttribute("href", dataStr); dlAnchorElem.setAttribute("download", `Respaldo_ERP_Maya_${new Date().toISOString().split('T')[0]}.json`); dlAnchorElem.click(); App.ui.toast("Respaldo descargado");
        },
        ...
        async eliminarRegistroGenerico(nombreHoja, idFila, nombreEstadoApp) {
            ...
            App.ui.showLoader("Eliminando..."); const res = await App.api.fetch("eliminar_fila", { nombreHoja: nombreHoja, idFila: idFila }); App.ui.hideLoader();
            if(res.status === "success") { App.state[nombreEstadoApp] = App.state[nombreEstadoApp].filter(item => item.id !== idFila); App.ui.toast("Registro eliminado"); App.router.handleRoute(); } else { App.ui.toast("Error al eliminar"); }
        },
        async eliminarPedido(pedidoId) {
            ...
            // Regresar inventario Multi-Material
            if(detalle) {
        ...
        async eliminarCompra(compraId) {
            if(!confirm("⚠️ ¿Eliminar compra? Deberás ajustar manualmente tu inventario restando la cantidad comprada.")) return;
            App.ui.showLoader("Eliminando..."); const res = await App.api.fetch("eliminar_fila", { nombreHoja: "compras", idFila: compraId }); App.ui.hideLoader();
            if(res.status === "success") { App.state.compras = App.state.compras.filter(c => c.id !== compraId); App.ui.toast("Compra eliminada."); App.router.handleRoute(); } else { App.ui.toast("Error al eliminar"); }
        },

        // --- SISTEMA DE GUARDADO ---
        async actualizarRegistroGenerico(nombreHoja, idFila, datosNuevos, nombreEstadoApp) {
            App.ui.showLoader("Guardando cambios..."); const res = await App.api.fetch("actualizar_fila", { nombreHoja: nombreHoja, idFila: idFila, datosNuevos: datosNuevos }); App.ui.hideLoader();
            if (res.status === "success") { const index = App.state[nombreEstadoApp].findIndex(item => item.id === idFila); if (index !== -1) App.state[nombreEstadoApp][index] = { ...App.state[nombreEstadoApp][index], ...datosNuevos }; App.ui.toast("Actualizado correctamente"); App.router.handleRoute(); } else { App.ui.toast("Error al actualizar"); }
        },
        async guardarNuevoGenerico(nombreHoja, datos, prefijoID, nombreEstadoApp) {
            App.ui.showLoader("Registrando..."); datos.id = prefijoID + "-" + Date.now(); if(!datos.fecha_creacion) datos.fecha_creacion = new Date().toISOString(); const res = await App.api.fetch("guardar_fila", { nombreHoja: nombreHoja, datos: datos }); App.ui.hideLoader();
            if (res.status === "success") { App.state[nombreEstadoApp].push(datos); App.ui.toast("Registro exitoso"); App.router.handleRoute(); } else { App.ui.toast("Error al guardar"); }
        },

        // --- MAGIA: PEDIDOS MULTI-MATERIAL ---
        async guardarNuevoPedido(datosFormulario) { 
            App.ui.showLoader("Creando pedido y apartando inventario..."); const pedidoId = "PED-" + Date.now(); const totalNum = parseFloat(datosFormulario.total) || 0; const anticipoNum = parseFloat(datosFormulario.anticipo) || 0; const cantidadNum = parseInt(datosFormulario.cantidad) || 1; 
        ...
        // --- CIERRE DE ORDEN MULTI-MATERIAL ---
        async cerrarOrdenProduccion(datosFormulario) {
            App.ui.showLoader("Ajustando mermas y nómina..."); const ordenId = datosFormulario.orden_id; const pagoArtesano = parseFloat(datosFormulario.pago_artesano) || 0; 
            ...
            for(let i=1; i<=3; i++) {
                ...
                if(matId && !isNaN(consumoTeorico) && !isNaN(consumoReal)) {
                    ...
                    if(material) {
                        ...
                        await App.api.fetch("guardar_fila", { nombreHoja: "consumos_produccion", datos: consumoObj }); App.state.consumos_produccion.push(consumoObj);
                    }
                }
            }

            if (orden.artesano_id && pagoArtesano > 0) { const pagoObj = { id: "PAGO-" + Date.now(), artesano_id: orden.artesano_id, orden_id: ordenId, monto_unitario: pagoArtesano, total: pagoArtesano, estado: "pendiente", fecha: new Date().toISOString() }; await App.api.fetch("guardar_fila", { nombreHoja: "pago_artesanos", datos: pagoObj }); App.state.pago_artesanos.push(pagoObj); }
            App.ui.hideLoader(); App.ui.toast("¡Orden finalizada con éxito!"); App.router.handleRoute();
        },

        async procesarCambioOrden(ordenId, datos) { if (datos.estado === 'listo') { App.views.formCerrarOrden(ordenId, datos.artesano_id); } else { App.ui.showLoader("Actualizando orden..."); const res = await App.api.fetch("actualizar_fila", { nombreHoja: "ordenes_produccion", idFila: ordenId, datosNuevos: datos }); App.ui.hideLoader(); if (res.status === "success") { const ordenIndex = App.state.ordenes_produccion.findIndex(o => o.id === ordenId); App.state.ordenes_produccion[ordenIndex] = { ...App.state.ordenes_produccion[ordenIndex], ...datos }; App.ui.toast("Orden actualizada"); App.router.handleRoute(); } else { App.ui.toast("Error al actualizar"); } } },
        async liquidarNomina(artesanoId) { App.ui.showLoader("Liquidando pagos..."); const pagosPendientes = App.state.pago_artesanos.filter(p => p.artesano_id === artesanoId && p.estado === 'pendiente'); let totalPagado = 0; const promesasDePago = []; for (let pago of pagosPendientes) { promesasDePago.push( App.api.fetch("actualizar_fila", { nombreHoja: "pago_artesanos", idFila: pago.id, datosNuevos: { estado: 'pagado' } }).then(res => { if(res.status === "success") { pago.estado = 'pagado'; totalPagado += parseFloat(pago.total || 0); } }) ); } await Promise.all(promesasDePago); if (totalPagado > 0) { const artesano = App.state.artesanos.find(a => a.id === artesanoId); const gastoObj = { id: "GAS-" + Date.now(), descripcion: "Nómina - " + (artesano ? artesano.nombre : 'Artesano'), categoria: "nomina", monto: totalPagado, fecha: new Date().toISOString().split('T')[0] }; await App.api.fetch("guardar_fila", { nombreHoja: "gastos", datos: gastoObj }); App.state.gastos.push(gastoObj); } App.ui.hideLoader(); App.ui.toast(`Se liquidaron $${totalPagado}`); App.router.handleRoute(); },
        
        // --- MAGIA: COMPRA DE REVENTA GENERA PRODUCTO AUTOMÁTICO ---
        async guardarNuevaCompra(datos) { 
            App.ui.showLoader("Registrando..."); const compra = { id: "COM-" + Date.now(), proveedor_id: datos.proveedor_id, fecha: datos.fecha, total: parseFloat(datos.total), monto_pagado: parseFloat(datos.total), estado: "pagado", fecha_creacion: new Date().toISOString() }; 
            const res = await App.api.fetch("guardar_fila", { nombreHoja: "compras", datos: compra }); 
            if (res.status === "success") { 
                App.state.compras.push(compra); const material = App.state.inventario.find(m => m.id === datos.material_id); 
                if(material) { 
                    const nuevoStock = parseFloat(material.stock_actual) + parseFloat(datos.cantidad); 
                    ...
                    // Si es Reventa, autogenerar Producto si no existe
                    if (material.tipo === 'reventa') {
                        ...
                        if(!existeProd) {
                            ...
                            App.ui.toast("Se auto-generó el producto de reventa.");
                        }
                    }
                } 
                App.ui.hideLoader(); App.ui.toast("Compra y stock actualizados"); App.router.handleRoute(); 
            } else { App.ui.hideLoader(); App.ui.toast("Error"); } 
        },
	...
	views: {
        ...
        inventario() { 
            ...
            let html = `<div class="card"><h3 class="card-title">Bodega / Inventario</h3><p style="color: var(--text-muted); font-size: 0.85rem; margin-bottom: 15px;">Hilos, Accesorios y Reventa.</p>`; 
            if (!App.state.inventario || App.state.inventario.length === 0) { html += `<p>No hay insumos registrados.</p>`; } else {
                html += `<table style="width:100%; border-collapse: collapse; font-size: 0.9rem;"><tr style="border-bottom: 2px solid var(--border);"><th style="text-align:left; padding:8px;">Artículo / Tipo</th><th style="padding:8px;">Stock</th><th></th></tr>`; 
                App.state.inventario.forEach(i => { html += `<tr style="border-bottom: 1px solid var(--border);"><td style="padding: 10px 5px;"><strong>${i.nombre}</strong><br><small style="color:var(--primary); text-transform:uppercase; font-size:0.75rem;">[${i.tipo || 'OTRO'}]</small> <small style="color:var(--text-muted)">${i.unidad}</small></td><td style="padding: 10px 5px; text-align:center; font-weight:bold; font-size:1.1rem; color:${i.stock_actual < 5 ? 'var(--danger)' : 'var(--text-main)'}">${i.stock_actual}</td><td style="text-align:right;"><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; margin-right:4px;" onclick="App.views.formMaterial('${i.id}')">✏️</button><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; color:red; border-color:red;" onclick="App.logic.eliminarRegistroGenerico('materiales', '${i.id}', 'inventario')">🗑️</button></td></tr>`; }); 
                html += `</table>`;
            }
        ...
        // --- ARTESANOS CON BOTÓN TARIFAS ---
        artesanos() { 
            document.getElementById('bottom-nav').style.display = 'flex'; 
            let html = `<div class="card"><h3 class="card-title">Directorio de Artesanos</h3>`; 
            App.state.artesanos.forEach(a => { 
                html += `<div class="card" style="border: 1px solid var(--border); padding: 10px; margin-bottom: 8px; display:flex; justify-content:space-between; align-items:center;"><div><strong>${a.nombre}</strong><br><small style="color: var(--text-muted);">📞 ${a.telefono || 'N/A'}</small></div><div><button class="btn btn-primary" style="padding:4px 8px; font-size:0.8rem; margin-right:4px; background:#D69E2E; border-color:#D69E2E;" onclick="App.views.verTarifasArtesano('${a.id}')">Tarifas 💲</button><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; margin-right:4px;" onclick="App.views.formArtesano('${a.id}')">✏️</button><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; color:red; border-color:red;" onclick="App.logic.eliminarRegistroGenerico('artesanos', '${a.id}', 'artesanos')">🗑️</button></div></div>`; 
            }); 
            html += `</div><button class="fab" onclick="App.views.formArtesano()">+</button>`; return html; 
        },
        
        // --- PANTALLA SECUNDARIA DE TARIFAS ---
        verTarifasArtesano(artesanoId) {
            ...
            let html = `
                <div style="margin-bottom: 15px;">Gestiona cuánto cobra <strong>${artesano.nombre}</strong> por cada tipo de hamaca.</div>
                <table style="width:100%; border-collapse: collapse; font-size: 0.9rem; margin-bottom:15px;">
                    <tr style="border-bottom: 2px solid var(--border);"><th style="text-align:left; padding:8px;">Clasificación</th><th style="padding:8px;">Monto</th><th></th></tr>
            `;
            if(tarifas.length === 0) { html += `<tr><td colspan="3" style="padding:10px; color:#aaa; text-align:center;">Sin tarifas</td></tr>`; }
            else {
                tarifas.forEach(t => {
            ...
            html += `</table><button class="btn btn-primary" style="width:100%;" onclick="App.ui.closeSheet(); setTimeout(()=>App.views.formTarifa('${artesanoId}'), 400);">+ Agregar Tarifa</button>`;
            App.ui.openSheet(`Tarifas de ${artesano.nombre}`, html);
        },

        compras() { document.getElementById('bottom-nav').style.display = 'flex'; let html = `<div class="card"><h3 class="card-title">Historial de Compras</h3>`; [...(App.state.compras||[])].reverse().forEach(c => { const p = App.state.proveedores.find(prv => prv.id === c.proveedor_id); html += `<div class="card" style="border: 1px solid var(--border); box-shadow: none;"><div style="display: flex; justify-content: space-between; margin-bottom: 5px;"><strong>${c.id}</strong><span style="color: var(--danger); font-weight: bold;">$${c.total}</span></div><p style="color: var(--text-muted); font-size: 0.85rem; margin-bottom: 5px;">Proveedor: ${p ? p.nombre : 'General'} | Fecha: ${c.fecha}</p><div style="text-align: right;"><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; color:red; border-color:red;" onclick="App.logic.eliminarCompra('${c.id}')">🗑️ Eliminar</button></div></div>`; }); return html += `</div><button class="fab" onclick="App.views.formCompra()">+</button>`; },
        ...
        reparaciones() { document.getElementById('bottom-nav').style.display = 'flex'; let html = `<div class="card"><h3 class="card-title">Reparaciones</h3>`; [...App.state.reparaciones].reverse().forEach(r => { const c = App.state.clientes.find(cli => cli.id === r.cliente_id); html += `<div class="card" style="border: 1px solid var(--border); box-shadow: none;"><div style="display: flex; justify-content: space-between; margin-bottom: 8px;"><strong>${r.id} - ${c ? c.nombre : 'Cliente'}</strong><span class="badge ${r.estado}">${r.estado.toUpperCase()}</span></div><p style="color: var(--primary); font-size: 0.9rem; font-weight: bold; margin-bottom: 5px;">${r.descripcion}</p><div style="text-align: right;"><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; color:red;" onclick="App.logic.eliminarRegistroGenerico('reparaciones', '${r.id}', 'reparaciones')">🗑️ Eliminar</button></div></div>`; }); return html += `</div><button class="fab" onclick="App.views.formNuevaReparacion()">+</button>`; },

        mas() { 
            document.getElementById('bottom-nav').style.display = 'flex'; 
            return `<div class="grid-2"><div class="card stat-card" style="cursor:pointer;" onclick="App.router.navigate('clientes')"><div class="label" style="margin-top:0;">👥 Clientes</div></div><div class="card stat-card" style="cursor:pointer;" onclick="App.router.navigate('proveedores')"><div class="label" style="margin-top:0;">🚚 Proveedores</div></div><div class="card stat-card" style="cursor:pointer;" onclick="App.router.navigate('artesanos')"><div class="label" style="margin-top:0;">🧑‍🎨 Artesanos / Tarifas</div></div><div class="card stat-card" style="cursor:pointer; background: #FFFBEB;" onclick="App.router.navigate('inventario')"><div class="label" style="margin-top:0; color:#D69E2E;">📦 Bodega / Insumos</div></div><div class="card stat-card" style="cursor:pointer;" onclick="App.router.navigate('productos')"><div class="label" style="margin-top:0;">🧶 Productos / Recetas</div></div><div class="card stat-card" style="cursor:pointer; background: #EBF8FF;" onclick="App.router.navigate('compras')"><div class="label" style="margin-top:0; color:#3182CE;">🛒 Ingresar Compra</div></div><div class="card stat-card" style="cursor:pointer; background: #FAF5FF;" onclick="App.router.navigate('reparaciones')"><div class="label" style="margin-top:0; color:#6B46C1;">🪡 Reparaciones</div></div><div class="card stat-card" style="cursor:pointer; background: #EDF2F7;" onclick="App.router.navigate('configuracion')"><div class="label" style="margin-top:0; color: #4A5568;">⚙️ Configuración</div></div></div>`; 
        }
    },
	// --- FORMULARIOS INTELIGENTES ---
        formMaterial(id = null) { const obj = id ? App.state.inventario.find(m => m.id === id) : null; const formHTML = `<form id="dynamic-form"><div style="background: #EBF8FF; padding: 10px; border-radius: 8px; margin-bottom: 15px; font-size: 0.85rem;">Crea un insumo (Ej. Hilo Rojo, Argollas, o Hamaca para Reventa).</div><div class="form-group"><label>Nombre del Insumo / Artículo</label><input type="text" name="nombre" value="${obj ? obj.nombre : ''}" required></div><div class="form-group"><label>Tipo de Insumo</label><select name="tipo" required><option value="hilo" ${obj && obj.tipo === 'hilo' ? 'selected' : ''}>Hilo para Tejer</option><option value="accesorio" ${obj && obj.tipo === 'accesorio' ? 'selected' : ''}>Accesorios (Argollas, etc)</option><option value="reventa" ${obj && obj.tipo === 'reventa' ? 'selected' : ''}>Hamaca Terminada (Reventa)</option><option value="otro" ${obj && obj.tipo === 'otro' ? 'selected' : ''}>Otro</option></select></div><div class="form-group"><label>Unidad de Medida</label><select name="unidad"><option value="Tubos" ${obj && obj.unidad === 'Tubos' ? 'selected' : ''}>Tubos</option><option value="Kg" ${obj && obj.unidad === 'Kg' ? 'selected' : ''}>Kilogramos (Kg)</option><option value="Pzas" ${obj && obj.unidad === 'Pzas' ? 'selected' : ''}>Piezas (Pzas)</option></select></div><div class="form-group"><label>Stock Actual</label><input type="number" step="0.1" name="stock_actual" value="${obj ? obj.stock_actual : '0'}" required></div><button type="submit" class="btn btn-primary" style="width: 100%;">${obj ? 'Guardar Cambios' : 'Crear Insumo'}</button></form>`; App.ui.openSheet(obj ? "Editar Insumo" : "Nuevo Insumo", formHTML, (data) => { if (obj) App.logic.actualizarRegistroGenerico("materiales", id, data, "inventario"); else App.logic.guardarNuevoGenerico("materiales", data, "MAT", "inventario"); }); },
        
        // --- RECETA MULTI-MATERIAL Y CLASIFICACIÓN ---
        formProducto(id = null) { 
            ...
            const formHTML = `
                <form id="dynamic-form">
                    ...
                    <div class="grid-2">
                        <div class="form-group"><label>Categoría</label><select name="categoria"><option value="fabricacion" ${obj && obj.categoria === 'fabricacion' ? 'selected' : ''}>Fabricación</option><option value="reventa" ${obj && obj.categoria === 'reventa' ? 'selected' : ''}>Reventa</option></select></div>
                        <div class="form-group"><label>Clasificación</label><select name="clasificacion"><option value="Unicolor" ${clasif==='Unicolor'?'selected':''}>Unicolor</option><option value="Combinada" ${clasif==='Combinada'?'selected':''}>Combinada</option><option value="Especial" ${clasif==='Especial'?'selected':''}>Especial</option><option value="Reventa" ${clasif==='Reventa'?'selected':''}>Reventa</option><option value="Otro" ${clasif==='Otro'?'selected':''}>Otro</option></select></div>
                    </div>
                    ...
                    <div style="background: #FFFBEB; padding: 10px; border-radius: 8px; margin-bottom: 15px;">
                        <strong style="color: #D69E2E; font-size: 0.9rem;">📦 Receta de Inventario</strong><br><small style="color: var(--text-muted);">Puedes descontar hasta 3 insumos al vender (Ej. Hilo Base, Hilo Borde y Argollas).</small>
                        <div class="grid-2" style="margin-top:10px;"><div class="form-group"><label>Insumo 1</label>${generarSelect('mat_1', obj ? (obj.mat_1 || obj.material_hilo_id) : '')}</div><div class="form-group"><label>Cant 1</label><input type="number" step="0.1" name="cant_1" value="${obj ? (obj.cant_1 || obj.tubos_hilo || '0') : '0'}"></div></div>
                        <div class="grid-2"><div class="form-group"><label>Insumo 2</label>${generarSelect('mat_2', obj ? obj.mat_2 : '')}</div><div class="form-group"><label>Cant 2</label><input type="number" step="0.1" name="cant_2" value="${obj ? obj.cant_2 : '0'}"></div></div>
                        <div class="grid-2"><div class="form-group"><label>Insumo 3</label>${generarSelect('mat_3', obj ? obj.mat_3 : '')}</div><div class="form-group"><label>Cant 3</label><input type="number" step="0.1" name="cant_3" value="${obj ? obj.cant_3 : '0'}"></div></div>
                    </div>
        ...
        formTarifa(artesanoId) {
            const formHTML = `<form id="dynamic-form"><input type="hidden" name="artesano_id" value="${artesanoId}"><div class="form-group"><label>Clasificación del Trabajo</label><select name="clasificacion" required><option value="Unicolor">Unicolor</option><option value="Combinada">Combinada</option><option value="Especial">Especial</option><option value="Otro">Otro</option></select></div><div class="form-group"><label>Monto a Pagar ($)</label><input type="number" step="0.01" name="monto" required></div><button type="submit" class="btn btn-primary" style="width: 100%; margin-top: 10px;">Guardar Tarifa</button></form>`;
            App.ui.openSheet("Nueva Tarifa", formHTML, (data) => App.logic.guardarNuevoGenerico("tarifas_artesano", data, "TAR", "tarifas_artesano"));
        },
        ...
        // --- CIERRE DE ORDEN MULTI-MATERIAL DINÁMICO ---
        formCerrarOrden(ordenId, artesanoAsignadoId) { 
            ...
            // Buscar tarifa por clasificación del producto
            const tarifa = App.state.tarifas_artesano.find(t => t.artesano_id === artesanoAsignadoId && t.clasificacion === producto.clasificacion); 
