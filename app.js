/**
 * DESCANSO MAYA ERP - Motor Principal v15
 * Compras, Inventario Automático y Reparaciones Integradas
 */
...
const App = {
    state: {
        ...
        compras: [], proveedores: [], reparaciones: [] // NUEVAS ENTIDADES
    },
    ...
    logic: {
        ...
        async cargarDatosIniciales() {
            ...
            try {
                // Descarga de las 12 hojas
                const hojas = ["materiales", "clientes", "productos", "pedidos", "pedido_detalle", "ordenes_produccion", "artesanos", "abonos_clientes", "gastos", "compras", "proveedores", "reparaciones"];
        ...
        // --- NUEVO: COMPRAS E INVENTARIO (Suma Stock) ---
        async guardarNuevaCompra(datos) {
            App.ui.showLoader("Registrando compra...");
            const compraId = "COM-" + Date.now();
            const compra = { id: compraId, proveedor_id: datos.proveedor_id, fecha: new Date().toISOString().split('T')[0], total: parseFloat(datos.total), monto_pagado: parseFloat(datos.total), estado: "pagado", fecha_creacion: new Date().toISOString() };
            ...
            if (res.status === "success") {
                ...
                App.ui.hideLoader(); App.ui.toast("Compra registrada y stock actualizado"); App.router.handleRoute();
            } else { App.ui.hideLoader(); App.ui.toast("Error al guardar compra"); }
        },
        ...
        async guardarNuevaReparacion(datos) {
            ...
            App.ui.hideLoader();
            if (res.status === "success") { App.state.reparaciones.push(datos); App.ui.toast("Reparación registrada"); App.router.handleRoute(); } 
            else { App.ui.toast("Error al guardar"); }
        },

        // --- MEJORA: CONSUMO AUTOMÁTICO (Resta Stock) ---
        async actualizarOrdenProduccion(ordenId, nuevosDatos) { 
            ...
            const ordenAnterior = App.state.ordenes_produccion[ordenIndex];
            
            const res = await App.api.fetch("actualizar_fila", { nombreHoja: "ordenes_produccion", idFila: ordenId, datosNuevos: nuevosDatos }); 
            
            if (res.status === "success") { 
                App.state.ordenes_produccion[ordenIndex] = { ...ordenAnterior, ...nuevosDatos }; 
                
                // Si se marca como LISTO, descontar material
                if(nuevosDatos.estado === 'listo' && ordenAnterior.estado !== 'listo') {
                    const detalle = App.state.pedido_detalle.find(d => d.id === ordenAnterior.pedido_detalle_id);
    ...
    views: {
        ...
        reparaciones() {
            ...
            let html = `<div class="card"><h3 class="card-title">Módulo de Reparaciones</h3>`;
            if (App.state.reparaciones.length === 0) html += `<p style="color: var(--text-muted);">No hay reparaciones registradas.</p>`;
            else {
                [...App.state.reparaciones].reverse().forEach(r => {
                    const c = App.state.clientes.find(cli => cli.id === r.cliente_id);
                    html += `<div class="card" style="border: 1px solid var(--border); box-shadow: none;"><div style="display: flex; justify-content: space-between; margin-bottom: 8px;"><strong>${r.id} - ${c ? c.nombre : 'Cliente'}</strong><span class="badge ${r.estado}">${r.estado.toUpperCase()}</span></div><p style="color: var(--primary); font-size: 0.9rem; font-weight: bold; margin-bottom: 5px;">${r.descripcion}</p><p style="color: var(--text-muted); font-size: 0.85rem;">Precio: $${r.precio} | Anticipo: $${r.anticipo}</p></div>`;
                });
            } return html += `</div><button class="fab" onclick="App.views.formNuevaReparacion()">+</button>`;
        },
        compras() {
            document.getElementById('bottom-nav').style.display = 'flex';
            let html = `<div class="card"><h3 class="card-title">Historial de Compras de Insumos</h3>`;
            if (App.state.compras.length === 0) html += `<p style="color: var(--text-muted);">No hay compras registradas.</p>`;
            else {
            ...
            } return html += `</div><button class="fab" onclick="App.views.formNuevaCompra()">+</button>`;
        },
