/**
 * DESCANSO MAYA ERP - Motor Principal v24
 * Corrección de Pantalla Blanca, Inventario Real y Ticket Premium
 */
...
const App = {
    ...
    logic: {
        ...
        async guardarNuevoGenerico(nombreHoja, datos, prefijoID, nombreEstadoApp) {
            App.ui.showLoader("Registrando..."); datos.id = prefijoID + "-" + Date.now();
            ...
            App.ui.hideLoader();
            if (res.status === "success") { App.state[nombreEstadoApp].push(datos); App.ui.toast("Registro exitoso"); App.router.handleRoute(); } 
        ...
        async guardarNuevoPedido(datosFormulario) { 
            ...
            const totalNum = parseFloat(datosFormulario.total) || 0; const anticipoNum = parseFloat(datosFormulario.anticipo) || 0; const cantidadNum = parseInt(datosFormulario.cantidad) || 1; 
            ...
            let materialAfectado = null; let nuevoStock = 0;
        ...
        async cerrarOrdenProduccion(datosFormulario) {
            App.ui.showLoader("Ajustando mermas y nómina...");
            const ordenId = datosFormulario.orden_id; const consumoReal = parseFloat(datosFormulario.consumo_real) || 0; const consumoTeorico = parseFloat(datosFormulario.consumo_teorico) || 0; const pagoArtesano = parseFloat(datosFormulario.pago_artesano) || 0;
            const orden = App.state.ordenes_produccion.find(o => o.id === ordenId); const detalle = App.state.pedido_detalle.find(d => d.id === orden.pedido_detalle_id); const producto = App.state.productos.find(p => p.id === detalle.producto_id); const material = producto ? App.state.inventario.find(m => m.id === producto.material_hilo_id) : null;
        ...
        async procesarCambioOrden(ordenId, datos) {
            if (datos.estado === 'listo') { App.views.formCerrarOrden(ordenId, datos.artesano_id); } 
                ...
                App.ui.showLoader("Actualizando orden..."); const res = await App.api.fetch("actualizar_fila", { nombreHoja: "ordenes_produccion", idFila: ordenId, datosNuevos: datos }); 
                App.ui.hideLoader();
                if (res.status === "success") { 
        ...
        async liquidarNomina(artesanoId) {
            ...
            for (let pago of pagosPendientes) { promesasDePago.push( App.api.fetch("actualizar_fila", { nombreHoja: "pago_artesanos", idFila: pago.id, datosNuevos: { estado: 'pagado' } }).then(res => { if(res.status === "success") { pago.estado = 'pagado'; totalPagado += parseFloat(pago.total || 0); } }) ); }
        ...
        async guardarNuevaCompra(datos) { 
            ...
            if (res.status === "success") { 
                ...
                const material = App.state.inventario.find(m => m.id === datos.material_id); 
                if(material) { 
        ...
        async mandarAProduccion(pedidoId) { 
            App.ui.showLoader("Generando Orden..."); const detalle = App.state.pedido_detalle.find(d => d.pedido_id === pedidoId); if (!detalle) return; 
            ...
            App.ui.hideLoader(); if (res.status === "success") { App.state.ordenes_produccion.push(nuevaOrden); App.ui.toast("Enviado a producción"); App.router.navigate('produccion'); } 
        },
	...
	views: {
        ...
        productos() { 
            ...
            if (App.state.productos.length === 0) { html += `<p style="color: var(--text-muted);">No hay productos registrados.</p>`; } 
                ...
                App.state.productos.forEach(p => { 
                    ...
                    html += `<div class="card" style="border: 1px solid var(--border); box-shadow: none; padding: 12px; display:flex; justify-content:space-between; align-items:center;"><div style="flex:1;"><strong>${p.nombre}</strong><br><small style="color: var(--text-muted);">Cat: ${p.categoria || 'General'}</small><br><small style="color: #d69e2e; font-weight:bold; font-size:0.8rem;">📦 ${recetaTexto}</small></div><div style="text-align:right;"><span style="color: var(--primary); font-weight: bold; display:block; margin-bottom:5px;">$${p.precio_venta}</span><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem;" onclick="App.views.formProducto('${p.id}')">✏️</button></div></div>`; 
                }); 
            } 
        ...
        formProducto(id = null) { 
            ...
            const formHTML = `<form id="dynamic-form"><div class="form-group"><label>Nombre del Producto a Vender</label><input type="text" name="nombre" value="${obj ? obj.nombre : ''}" required></div><div class="form-group"><label>Categoría</label><select name="categoria"><option value="fabricacion" ${obj && obj.categoria === 'fabricacion' ? 'selected' : ''}>Fabricación (Taller)</option><option value="reventa" ${obj && obj.categoria === 'reventa' ? 'selected' : ''}>Reventa (Ya hecha)</option></select></div><div class="form-group"><label>Precio de Venta ($)</label><input type="number" name="precio_venta" value="${obj ? obj.precio_venta : ''}" required></div><div style="background: #FFFBEB; padding: 10px; border-radius: 8px; margin-bottom: 15px;"><strong style="color: #D69E2E; font-size: 0.9rem;">📦 Receta de Inventario (Al vender)</strong><br><small style="color: var(--text-muted);">¿Qué y cuánto se descuenta de tu bodega al vender 1 unidad de este producto?</small><div class="form-group" style="margin-top:10px;"><label>Material / Artículo a descontar</label><select name="material_hilo_id" required><option value="">-- Elige Insumo --</option>${opcMat}</select></div><div class="form-group"><label>Cantidad a descontar</label><input type="number" step="0.1" name="tubos_hilo" value="${obj ? obj.tubos_hilo : '1'}" required></div></div><button type="submit" class="btn btn-primary" style="width: 100%;">${obj ? 'Guardar Cambios' : 'Crear Producto'}</button></form>`; 
