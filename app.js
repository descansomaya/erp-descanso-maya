...
App.views = {
    ...
    produccion() { 
        document.getElementById('bottom-nav').style.display = 'flex'; 
        let html = `<div class="card"><h3 class="card-title">Tablero Kanban</h3><button class="btn btn-primary" style="width:100%; margin-top:10px; background:#D69E2E; border-color:#D69E2E;" onclick="App.views.formOrdenStock()">+ 🔨 Fabricar para Bodega</button></div>`; 
        ...
        const dibujarTarjeta = (orden) => { 
            ...
            return `<div class="kanban-card" onclick="App.views.modalEditarOrden('${orden.id}')"><div style="display:flex; justify-content:space-between; margin-bottom:5px;"><strong>${orden.id}</strong><span style="font-size:0.75rem; color:#718096; font-weight:bold;">${pedido ? pedido.id.replace('PED-','') : ''}</span></div><small style="color: var(--primary); font-weight: 600; display:block;">${producto ? producto.nombre : 'Producto interno'}</small><small style="color: #4A5568; display:block; margin-bottom:5px; font-weight:bold;">👤 ${nombreCliente}</small><div style="margin-top: 8px; font-size: 0.8rem; color: var(--text-muted); display:flex; justify-content: space-between; border-top: 1px dashed #E2E8F0; padding-top: 5px;"><span>${infoArtesanos}</span></div></div>`; 
        }; 
        
        html += `<div class="kanban-board"><div class="kanban-column"><div class="kanban-header">Pendientes <span>${pendientes.length}</span></div>${pendientes.map(dibujarTarjeta).join('')}</div><div class="kanban-column"><div class="kanban-header">En Proceso <span>${enProceso.length}</span></div>${enProceso.map(dibujarTarjeta).join('')}</div><div class="kanban-column"><div class="kanban-header">Listas <span>${listas.length}</span></div>${listas.map(dibujarTarjeta).join('')}</div></div>`; 
        return html; 
    },
    ...
    finanzas() { 
        ...
        if (!App.state.gastos || App.state.gastos.length === 0) { 
            ...
            [...App.state.gastos].reverse().slice(0, 20).forEach(g => { 
                html += `<div style="display: flex; justify-content: space-between; border-bottom: 1px solid var(--border); padding: 10px 0;"><div><strong style="font-size: 0.95rem;">${g.descripcion}</strong><span style="cursor:pointer; margin-left: 5px; font-size: 0.8rem;" onclick="App.views.formGasto('${g.id}')">✏️</span><span style="cursor:pointer; margin-left: 5px; font-size: 0.8rem; color:red;" onclick="App.logic.eliminarRegistroGenerico('gastos', '${g.id}', 'gastos')">🗑️</span><br><small style="color: var(--text-muted);">${g.categoria} | ${g.fecha}</small></div><strong style="color: var(--danger); font-size: 1rem;">-$${g.monto}</strong></div>`; 
            }); 
        } 
    ...
    reparaciones() { 
        ...
        [...App.state.reparaciones].reverse().forEach(r => { 
            ...
            html += `<div class="card" style="border: 1px solid var(--border); box-shadow: none;"><div style="display: flex; justify-content: space-between; margin-bottom: 8px;"><strong>${r.id} - ${c ? c.nombre : 'Cliente'}</strong><span class="badge ${estaLista ? 'listo' : 'pendiente'}">${r.estado.toUpperCase()}</span></div><p style="color: var(--primary); font-size: 0.9rem; font-weight: bold; margin-bottom: 5px;">${r.descripcion}</p><div style="font-size:0.8rem; margin-bottom:10px; color:#4A5568;">${infoArtesanos}</div><div style="display:flex; justify-content:space-between; gap:5px;">`; 
            if(!estaLista) { 
                html += `<button class="btn btn-secondary" style="font-size:0.8rem; padding:6px; flex:1;" onclick="App.views.formAgregarTrabajo('${r.id}', true)">+ Asignar Tarea</button>`; 
            ...
            html += `<button class="btn btn-secondary" style="padding:6px; font-size:0.8rem; color:red;" onclick="App.logic.eliminarRegistroGenerico('reparaciones', '${r.id}', 'reparaciones')">🗑️</button></div></div>`; 
        }); 
    ...
    cotizaciones() { 
        ...
        if (!App.state.cotizaciones || App.state.cotizaciones.length === 0) { 
            ...
            [...App.state.cotizaciones].reverse().forEach(c => { 
                html += `<div class="card" style="border: 1px solid var(--border); box-shadow: none;"><div style="display: flex; justify-content: space-between; margin-bottom: 5px;"><strong>${c.id} - ${c.cliente_nombre}</strong><span style="color: var(--primary); font-weight: bold;">$${c.total}</span></div><p style="color: var(--text-muted); font-size: 0.85rem; margin-bottom: 5px;">${c.descripcion}</p><div style="display:flex; gap:5px; justify-content:flex-end;"><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; margin-right:4px;" onclick="App.logic.imprimirCotizacion('${c.id}')">🖨️ Imprimir PDF</button><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; color:red; border-color:red;" onclick="App.logic.eliminarCotizacion('${c.id}')">🗑️</button></div></div>`; 
            }); 
        } 
    ...
    clientes() { 
        ...
        App.state.clientes.forEach(c => { html += `<div class="card" style="border: 1px solid var(--border); padding: 10px; margin-bottom: 8px; display:flex; justify-content:space-between; align-items:center;"><div><strong>${c.nombre}</strong><br><small style="color: var(--text-muted);">📞 ${c.telefono || 'N/A'}</small></div><div><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; margin-right:4px;" onclick="App.views.formCliente('${c.id}')">✏️</button><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; color:red; border-color:red;" onclick="App.logic.eliminarRegistroGenerico('clientes', '${c.id}', 'clientes')">🗑️</button></div></div>`; }); 
    ...
    proveedores() { 
        ...
        App.state.proveedores.forEach(p => { html += `<div class="card" style="border: 1px solid var(--border); padding: 10px; margin-bottom: 8px; display:flex; justify-content:space-between; align-items:center;"><div><strong>${p.nombre}</strong><br><small style="color: var(--text-muted);">📞 ${p.telefono || 'N/A'}</small></div><div><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; margin-right:4px;" onclick="App.views.formProveedor('${p.id}')">✏️</button><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; color:red; border-color:red;" onclick="App.logic.eliminarRegistroGenerico('proveedores', '${p.id}', 'proveedores')">🗑️</button></div></div>`; }); 
    ...
    compras() { 
        ...
        [...(App.state.compras||[])].reverse().forEach(c => { 
            ...
            html += `<div class="card" style="border: 1px solid var(--border); box-shadow: none;"><div style="display: flex; justify-content: space-between; margin-bottom: 5px;"><strong>${c.id}</strong><span style="color: var(--danger); font-weight: bold;">$${c.total}</span></div><p style="color: var(--text-muted); font-size: 0.85rem; margin-bottom: 5px;">Proveedor: ${p ? p.nombre : 'General'} | Fecha: ${c.fecha}<br><em>(Varios Insumos)</em></p><div style="text-align: right;"><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; margin-right:4px;" onclick="App.views.formEditarCompra('${c.id}')">✏️ Editar</button><button class="btn btn-secondary" style="padding:4px 8px; font-size:0.8rem; color:red; border-color:red;" onclick="App.logic.eliminarCompra('${c.id}')">🗑️ Eliminar</button></div></div>`; 
        }); 
...
App.views.formMaterial = function(id = null) { 
    ...
    App.ui.openSheet(obj ? "Editar Insumo" : "Nuevo Insumo", formHTML, (data) => { if (obj) App.logic.actualizarRegistroGenerico("materiales", id, data, "inventario"); else App.logic.guardarNuevoGenerico("materiales", data, "MAT", "inventario"); }); 
};
...
App.views.formTarifa = function(artesanoId) { 
...
App.views.formCliente = function(id = null) { 
...
App.views.formProveedor = function(id = null) { 
    ...
    App.ui.openSheet(obj ? "Editar Proveedor" : "Nuevo Proveedor", formHTML, (data) => { if (obj) App.logic.actualizarRegistroGenerico("proveedores", id, data, "proveedores"); else App.logic.guardarNuevoGenerico("proveedores", data, "PRV", "proveedores"); }); 
};
...
App.views.formCompra = function() { 
    ...
    let itemsHTML = ''; 
    for(let i=1; i<=5; i++) { 
        itemsHTML += `<div class="grid-2" style="margin-bottom: 5px;"><div class="form-group" style="margin:0;"><label>Insumo ${i}</label><select name="mat_${i}"><option value="">-- Ninguno --</option>${opcMat}</select></div><div class="form-group" style="margin:0;"><label>Cantidad</label><input type="number" step="0.1" name="cant_${i}"></div></div>`; 
    } 
    const formHTML = `
...
App.views.modalEditarOrden = function(ordenId) { 
    ...
    pagosHTML += `<div style="margin-top: 10px; text-align: right; font-weight: bold; color: var(--primary);">Costo Mano de Obra: $${costoManoObra.toFixed(2)}</div></div><button type="button" class="btn btn-secondary" style="width:100%; margin-bottom:15px; border-style:dashed;" onclick="App.ui.closeSheet(); setTimeout(()=>App.views.formAgregarTrabajo('${ordenId}', false), 300);">+ Asignar Tarea / Artesano</button>`; 
...
App.views.formCerrarOrden = function(ordenId) { 
    ...
    for(let i=1; i<=5; i++){ 
        const matId = producto[`mat_${i}`]; 
        const cant = parseFloat(producto[`cant_${i}`]); 
        const uso = producto[`uso_${i}`] || 'General'; 
        if(matId && cant > 0) { 
            const material = App.state.inventario.find(m => m.id === matId); 
