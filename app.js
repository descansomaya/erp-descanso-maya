/**
 * DESCANSO MAYA ERP - Motor Principal v20
 * Restauración de Vistas (Clientes, Productos, Finanzas) + Cierre de Orden
 */
...
const App = {
    ...
    views: {
        ...
        // --- SE RESTAURA EL DASHBOARD FINANCIERO COMPLETO (Gastos + Botón) ---
        finanzas() { 
            ...
            if (App.state.gastos.length === 0) { 
                ...
                [...App.state.gastos].reverse().slice(0, 15).forEach(g => { 
                    html += `<div style="display: flex; justify-content: space-between; border-bottom: 1px solid var(--border); padding: 10px 0;"><div><strong style="font-size: 0.95rem;">${g.descripcion}</strong><br><small style="color: var(--text-muted);">${g.categoria} | ${g.fecha}</small></div><strong style="color: var(--danger); font-size: 1rem;">-$${g.monto}</strong></div>`; 
                }); 
            }
        ...
        // --- SE RESTAURA EL DIRECTORIO DE PRODUCTOS COMPLETO (Categorías + Botón) ---
        productos() { 
            document.getElementById('bottom-nav').style.display = 'flex'; 
            let html = `<div class="card"><h3 class="card-title">Catálogo</h3>`; 
            if (App.state.productos.length === 0) {
            ...
            html += `</div><button class="fab" onclick="App.views.formNuevoProducto()">+</button>`; 
            return html; 
        },

        // --- SE RESTAURA EL DIRECTORIO DE CLIENTES COMPLETO (Detalles + Botón) ---
        clientes() { 
            document.getElementById('bottom-nav').style.display = 'flex'; 
            let html = `<div class="card"><h3 class="card-title">Directorio de Clientes</h3>`; 
            if (App.state.clientes.length === 0) {
                html += `<p style="color: var(--text-muted);">No hay clientes registrados.</p>`;
            } else {
                App.state.clientes.forEach(c => { 
                    html += `<div class="card" style="border: 1px solid var(--border); padding: 10px; margin-bottom: 8px;"><strong>${c.nombre}</strong><br><small style="color: var(--text-muted);">📞 ${c.telefono || 'Sin teléfono'} | 📍 ${c.direccion || 'Sin dirección'}</small></div>`; 
                }); 
            }
            html += `</div><button class="fab" onclick="App.views.formNuevoCliente()">+</button>`; 
            return html; 
        },
